...
Sceen::Sceen(DifficultyLevel diff) : difficulty(diff), points_per_success(get_points_for_level()) {}
...
void Sceen::update_available_actions() {
    ...
    if (player.has_selected_toy()) {
        ...
        action_menu.add_action(std::make_unique<DeselectToyAction>());
    } else {
...
std::vector<std::string> Sceen::get_action_descriptions() const {
    ...
    for (std::size_t i = 0; i < action_menu.size(); ++i) {
...
void Sceen::execute_place_toy(std::size_t frame_index) {
    ...
    if (panel.try_place_toy(toy, frame_index)) {
        ...
        set_message(std::format("Успешно. + {} очков",points_per_success));
    } else {
